...
switch($data){
    ...
    // $keyboard=array("inline_keyboard"=>$inline_keyboard);
    // $replyMarkup = json_encode($keyboard); 
    sendMessage($chat_id_in,"[".date('H:i:s',$nice3)."] Пользователь @{$username_call}  {$user_first_name_group1} [<b>{$chat_id_in2}</b>] в групе [<b> {$nice2} </b>]   Проголосовал 👍 за {$nice} ",$msgid);
    break;
    case '/votedown':
    ...
    // $keyboard=array("inline_keyboard"=>$inline_keyboard);
    // $replyMarkup = json_encode($keyboard); 
    sendMessage($chat_id_in,"[".date('H:i:s',$nice3)."] Пользователь @{$username_call} {$user_first_name_group1} [<b>{$chat_id_in2}</b>] в [<b> {$nice2} </b>]  Проголосовал 👎 за {$nice} ",$msgid);
    break;
}
?>

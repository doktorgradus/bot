...
switch($data){
    ...
    // $keyboard=array("inline_keyboard"=>$inline_keyboard);
    // $replyMarkup = json_encode($keyboard); 
    sendMessage($chat_id_in,"[".date('H:i:s',$nice3)."] Пользователь @{$$username_call2}{$username_call}{$user_name_group_call2} {$user_first_name_group1} [<b>{$chat_id_in2}</b>] в {$nice2}   Проголосовал 👍 за {$nice} ",$msgid);
    break;
    case '/votedown':

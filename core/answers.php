...
switch($data){
  ...
  sendMessage($chat_id_in,"[".date('H:i:s',$nice3)."] Пользователь @{$user_name_group_call2} {$user_first_name_group1} [<b>{$chat_id_in2}</b>] в групе [<b> {$nice2} </b>]   Проголосовал 👍 за {$nice} ",$msgid+1);
}
    break;
    case '/votedown':
    ...
    // $keyboard=array("inline_keyboard"=>$inline_keyboard);
    // $replyMarkup = json_encode($keyboard); 
    sendMessage($chat_id_in,"[".date('H:i:s',$nice3)."] Пользователь @{$user_name_group_call2} {$user_first_name_group1} [<b>{$chat_id_in2}</b>] в [<b> {$nice2} </b>]  Проголосовал 👎 за {$nice} ",$msgid-1);
    break;
}

...
switch($data){
    ...
    // $keyboard=array("inline_keyboard"=>$inline_keyboard);
    // $replyMarkup = json_encode($keyboard); 
    sendMessage($chat_id_in,"Пользователь {$$username_call2} {$username_call} с ID: {$chat_id_in} {$chat_id_in2} в {$user_name_group_call} {$user_name_group_call2} {$user_first_name_group1}  Проголосовал за № {$rand} 👍",$msgid);
    break;
    case '/votedown':
    ...
    // $keyboard=array("inline_keyboard"=>$inline_keyboard);
    // $replyMarkup = json_encode($keyboard); 
    sendMessage($chat_id_in,"Пользователь {$username_call} с ID: {$chat_id_in} в {$user_name_group_call} {$user_name_group_call2} {$user_first_name_group1}Проголосовал 👎",$msgid);
    break;
}
?>
